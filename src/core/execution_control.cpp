...
void ExecutionControl::PreSetup() {
  ...
  knob_->RegisterStr("sinfo_in", "Static info input file.", "sinfo.db");
  knob_->RegisterStr("sinfo_out", "Static info output file.", "sinfo.db");
...
void ExecutionControl::PostSetup() {
  ...
  callstack_info_ = new CallStackInfo();
  ...
  for (BBL bbl = TRACE_BblHead(trace); BBL_Valid(bbl); bbl = BBL_Next(bbl)) {
    ...
    if (desc_.HookAtomicInst()) {
      for (INS ins = BBL_InsHead(bbl); INS_Valid(ins); ins = INS_Next(ins)) {
        ...
        if (INS_Opcode(ins) == XED_ICLASS_CMPXCHG ||
    ...
    if (desc_.HookCallStack()) {
      INS tail = BBL_InsTail(bbl);
      Inst *inst = GetInst(INS_Address(tail));
      if (INS_IsCall(tail)) {
...
void ExecutionControl::ThreadExit(THREADID tid, const CONTEXT *ctxt, INT32 code,
  ...
  // update call_stack_map_
  thread_id_t self = Self();
  callstack_info_->RemoveCallStack(self);
}
...
void ExecutionControl::HandleImageLoad(IMG img, Image *image) {
  ...
  for(SEC sec = IMG_SecHead(img); SEC_Valid(sec); sec = SEC_Next(sec)) {
    if (SEC_Type(sec) == SEC_TYPE_DATA) {
      data_start = SEC_Address(sec);
      data_size = SEC_Size(sec);
    }
    if (SEC_Type(sec) == SEC_TYPE_BSS) {
...
void ExecutionControl::HandlePthreadCreate(PthreadCreateContext *context) {
  ...
  // update call_stack return info.
  CallStack *call_stack = callstack_info_->GetCallStack(self);
  call_stack->ProcessReturn(context->ret_addr());  
}
...
void ExecutionControl::HandleAfterCall(Inst *inst, ADDRINT ret_pc) {
  thread_id_t self = Self();
  // update call_stack call info
  CallStack *call_stack = callstack_info_->GetCallStack(self);
  DEBUG_ASSERT(call_stack);
  call_stack->ProcessCall(inst, ret_pc);  
}

void ExecutionControl::HandleBeforeRet(ADDRINT target_pc) {
  thread_id_t self = Self();
  // update call_stack return info
  CallStack *call_stack = callstack_info_->GetCallStack(self);
  DEBUG_ASSERT(call_stack);
  call_stack->ProcessReturn(target_pc);  
}
...
thread_id_t ExecutionControl::GetThdID(pthread_t thread) {
  ...
  if (pthread_handle_map_.find(thread) == pthread_handle_map_.end())
    ...
    thd_id = pthread_handle_map_[thread];
  UnlockKernel();
...
thread_id_t ExecutionControl::GetParent() {
  thread_id_t parent = INVALID_THD_ID;
  OS_THREAD_ID parent_os_tid = PIN_GetParentTid();
  LockKernel();
  if (parent_os_tid)
    parent = os_tid_map_[parent_os_tid];
  UnlockKernel();
  return parent;
}
...
void ExecutionControl::NotifyNewChild() {
  ...
  thd_create_sem_map_[parent_os_tid]->Post();
  UnlockKernel();
}
...
void ExecutionControl::__AfterCall(Inst *inst, ADDRINT ret_pc) {
  ctrl_->HandleAfterCall(inst, ret_pc);
}

void ExecutionControl::__BeforeRet(ADDRINT target_pc) {
  ctrl_->HandleBeforeRet(target_pc);
}
