...
class ObserverAccess {
 public:
  ...
  ObserverAccess(thread_id_t thd_id, timestamp_t clk,
      		 ...
      		 std::vector<address_t> callstack_vector)
      : thd_id_(thd_id), clk_(clk), type_(type), inst_(inst),
        callers_vector_(callstack_vector) {}
  ~ObserverAccess() {}
 ...
 private:
  ...
  Inst *inst_;
  std::vector<address_t> callers_vector_;
...
class Observer : public Analyzer {
 public:
  ...
  void Register();
  bool Enabled();  
  void Setup(Mutex *lock, StaticInfo *sinfo, CallStackInfo *callstack_info,
             idiom::iRootDB *iroot_db, iList *ilist);  
