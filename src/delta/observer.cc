...
void Observer::Register() {
  ...
  knob_->RegisterBool("calltracer", "whether enable call-stack monitor", "0");
  knob_->RegisterInt("caller_level", "how many level traced back", "4");
}
...
void Observer::Setup(Mutex *lock, StaticInfo *sinfo, 
                     CallStackInfo *callstack_info,
                     idiom::iRootDB *iroot, iList *ilist) {
  internal_lock_ = lock;
  sinfo_ = sinfo;
  callstack_info_ = callstack_info;
  iroot_db_ = iroot;
  ilist_ = ilist;

  unit_size_ = knob_->ValueInt("unit_size"); 
  caller_level_ = knob_->ValueInt("caller_level");
  ...
  if (knob_->ValueBool("calltracer"))
    desc_.SetHookCallStack();
}
...
void Observer::ImageLoad(Image *image, address_t low_addr,
  ...
  DEBUG_ASSERT(data_start && bss_start);
...
void Observer::ImageUnload(Image *image, address_t low_addr,
                           ...
                           size_t data_size, address_t bss_start,
                           size_t bss_size) {
  DEBUG_ASSERT(low_addr && data_start && bss_start);
...
void Observer::UpdateForRead(thread_id_t curr_thd_id, timestamp_t curr_thd_clk,
  ...
  CallStack *call_stack_ = callstack_info_->GetCallStack(curr_thd_id);
  std::vector<address_t> callers_vector = call_stack_->GetCallerStack(caller_level_);
  ObserverAccess curr_access(curr_thd_id, curr_thd_clk,
                             idiom::IROOT_EVENT_MEM_READ, inst, callers_vector);
...
void Observer::UpdateForUnlock(thread_id_t curr_thd_id,
  ...
  std::vector<address_t> callers_vector = call_stack_->GetCallerStack(caller_level_);
  ObserverAccess curr_access(curr_thd_id, curr_thd_clk,
                             idiom::IROOT_EVENT_MUTEX_UNLOCK, 
                             inst, callers_vector);
  std::vector<ObserverAccess> preds;
...
void Observer::UpdateiRoots(ObserverAccess *curr_access,
  ...
  for (std::vector<ObserverAccess>::iterator it = preds->begin();
    ...
    ilist_->Update(iroot->id(), curr_access->inst_, 
                   curr_access->callers_vector_,
                   it->inst_, it->callers_vector_, true);
  }
}
      ...
      for (ObserverLocalInfo::SuccVec::iterator sit = succs->begin();
        ...
        if (sa.thd_id_ == pa.thd_id_ && sa.clk_ < pa.clk_ && type3_) {
         ...
         idiom::iRoot *iroot = iroot_db_->GetiRoot(idiom::IDIOM_3, true, iroot_event_pred,
                                                   ...
                                                   iroot_event_curr);
         ilist_->Update(iroot->id(),
                        curr_access->inst_, curr_access->callers_vector_,
                        pa.inst_, pa.callers_vector_,
                        sa.inst_, sa.callers_vector_,
                        prev_access->inst_, prev_access->callers_vector_,
                        true);
        }
